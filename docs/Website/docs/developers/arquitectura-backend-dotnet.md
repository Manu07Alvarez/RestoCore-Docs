...
* **`Features/PublicMenu`:** Consulta optimizada de la carta digital mediante `GetPublicMenuQuery`. Implementa caché en memoria de alta concurrencia con llaves versionadas por tenant y cálculo de hash criptográfico `ETag`.
...
* **Persistencia (`ApplicationDbContext`):**
  * Implementa Global Query Filters a nivel modelo (`builder.Entity<T>().HasQueryFilter(e => e.TenantId == _tenantContext.CurrentTenantId)`), blindando el aislamiento multi-tenant.
  * **Invalidación de Caché Post-Commit:** Detección de mutaciones en entidades del menú durante `SaveChangesAsync()` e invalidación atómica de la caché en memoria una vez confirmada la transacción en PostgreSQL, previniendo condiciones de carrera.
...
* **Middleware Multi-Tenant (`TenantResolutionMiddleware`):** Resuelve el `TenantId` activo evaluando jerárquicamente:
...
* **Mapeo Modular de Rutas:** Agrupación semántica (`PublicMenuEndpoints`, `AdminCatalogEndpoints`, `AdminTableEndpoints`, `AdminTenantEndpoints`, `KitchenEndpoints`, `StorageEndpoints`, `HealthEndpoints`).
* **Soporte de Caché Condicional:** Implementación de cabeceras HTTP `ETag` y `Cache-Control: public, max-age=60, stale-while-revalidate=300` con retorno HTTP 304 Not Modified ante coincidencias de `If-None-Match`.
...
El backend cuenta con 30 pruebas automatizadas organizadas en tres proyectos de validación:

### 5.1. Pruebas Unitarias (`RestoCore.UnitTests`) - 18 Tests Superados
...
* `PublicMenuQueryTests`: Valida la construcción jerárquica de la respuesta pública y consistencia del hash `ETag`.
* `QrCodeServiceTests`: Valida la generación correcta de matrices QR en SVG y PNG.

### 5.2. Pruebas de Integración (`RestoCore.IntegrationTests`) - 11 Tests Superados
* `ContainerReadinessTests`: Verifica la conectividad y respuesta saludable de dependencias externas.
* `SeaweedStorageTests`: Verifica la generación y expiración de URLs pre-firmadas.
...
### 5.3. Pruebas de Seguridad (`RestoCore.SecurityTests`) - 1 Test Superado
